...
class BCE(Loss):
    ...
    def forward(self, y_out, y_truth, individual_losses=False):
        ...
        losses = - (y_truth * np.log(y_out) + (1 - y_truth) * np.log(1 - y_out))
        
        # Aggregate losses across samples if individual_losses is False
        if not individual_losses:
            result = np.mean(losses)  # Mean of losses across samples
        else:
            result = losses  # Return individual losses for each sample
    ...
    def backward(self, y_out, y_truth):
        ...
        gradient /= len(y_out)  # Divide by the number of samples in the batch
