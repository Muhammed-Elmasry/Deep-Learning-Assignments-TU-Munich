...
class Classifier(Network):
    ...
    def forward(self, X):
        ...
        z = self.sigmoid(np.dot(X, self.W))  # Applying the sigmoid activation function
        self.cache = (X, z)  # Storing the relevant variables for backpropagation
    ...
    def backward(self, dout):
        ...
        X, z = self.cache
        dz = dout * z * (1 - z)  # Derivative of the sigmoid function
        dW = np.dot(X.T, dz)  # Computing the gradient of the weights
